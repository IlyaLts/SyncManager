...
void SyncManager::loadFromFileData(SyncFolder &folder, QDataStream &stream, bool dry)
{
    ...
    for (qsizetype k = 0; k < numOfFiles; k++)
    {
        ...
        if (!dry)
        {
            QPair<QByteArray, QPair<QByteArray, Attributes>> pair(to, QPair(from, attributes));
...
void SyncManager::synchronizeFileAttributes(SyncProfile &profile)
{
    ...
    for (auto folderIt = profile.folders.begin(); folderIt != profile.folders.end(); ++folderIt)
    {
        for (auto otherFolderIt = profile.folders.begin(); otherFolderIt != profile.folders.end(); ++otherFolderIt)
        {
            ...
            for (QHash<hash64_t, SyncFile>::iterator otherFileIt = otherFolderIt->files.begin(); otherFileIt != otherFolderIt->files.end(); ++otherFileIt)
            {
...
void SyncManager::checkForRenamedFolders(SyncProfile &profile)
{
    ...
    for (auto folderIt = profile.folders.begin(); folderIt != profile.folders.end(); ++folderIt)
    {
        ...
        for (QHash<hash64_t, SyncFile>::iterator renamedFolderIt = folderIt->files.begin(); renamedFolderIt != folderIt->files.end(); ++renamedFolderIt)
        {
            ...
            for (auto otherFolderIt = profile.folders.begin(); otherFolderIt != profile.folders.end(); ++otherFolderIt)
            {
                ...
                QPair<QByteArray, QPair<QByteArray, Attributes>> pair(renamedFolderIt->path, QPair(otherFolder.filePath().toUtf8(), getFileAttributes(folderPath)));
...
void SyncManager::checkForMovedFiles(SyncProfile &profile)
{
    ...
    for (auto folderIt = profile.folders.begin(); folderIt != profile.folders.end(); ++folderIt)
    {
        ...
        QHash<hash64_t, SyncFile *> missingFiles;
        QHash<hash64_t, SyncFile *> newFiles;

        // Finds files that don't exist in our sync folder anymore
        for (QHash<hash64_t, SyncFile>::iterator fileIt = folderIt->files.begin(); fileIt != folderIt->files.end(); ++fileIt)
        ...
        for (QHash<hash64_t, SyncFile>::iterator newFileIt = folderIt->files.begin(); newFileIt != folderIt->files.end(); ++newFileIt)
        ...
        for (QHash<hash64_t, SyncFile *>::iterator newFileIt = newFiles.begin(); newFileIt != newFiles.end(); ++newFileIt)
        {
            bool abort = false;
            ...
            for (QHash<hash64_t, SyncFile *>::iterator missingFileIt = missingFiles.begin(); missingFileIt != missingFiles.end(); ++missingFileIt)
            {
            ...
            for (auto otherFolderIt = profile.folders.begin(); otherFolderIt != profile.folders.end(); ++otherFolderIt)
            {
                ...
                otherFolderIt->filesToMove.insert(movedFileHash, QPair<QByteArray, QPair<QByteArray, Attributes>>(newFileIt.value()->path, QPair(pathToMove, getFileAttributes(pathToNewFile))));
...
void SyncManager::checkForRemovedFiles(SyncProfile &profile)
{
    ...
    for (auto folderIt = profile.folders.begin(); folderIt != profile.folders.end(); ++folderIt)
    {
        for (QHash<hash64_t, SyncFile>::iterator fileIt = folderIt->files.begin() ; fileIt != folderIt->files.end();)
        {
            if (!folderIt->isActive())
                break;
            ...
            {
                fileIt = folderIt->files.erase(static_cast<QHash<hash64_t, SyncFile>::const_iterator>(fileIt));
                continue;
            }
...
void SyncManager::renameFolders(SyncFolder &folder)
{
    for (auto folderIt = folder.foldersToRename.begin(); folderIt != folder.foldersToRename.end() && (!m_paused && folder.isActive());)
    {
        ...
        {
            folderIt = folder.foldersToRename.erase(static_cast<QHash<hash64_t, QPair<QByteArray, QPair<QByteArray, Attributes>>>::const_iterator>(folderIt));
            continue;
        }
...
void SyncManager::moveFiles(SyncFolder &folder)
{
    for (auto fileIt = folder.filesToMove.begin(); fileIt != folder.filesToMove.end() && (!m_paused && folder.isActive());)
    {
        ...
        {
            fileIt = folder.filesToMove.erase(static_cast<QHash<hash64_t, QPair<QByteArray, QPair<QByteArray, Attributes>>>::const_iterator>(fileIt));
            continue;
        }
...
void SyncManager::createFolders(SyncFolder &folder, const QString &versioningPath)
{
    for (auto folderIt = folder.foldersToCreate.begin(); folderIt != folder.foldersToCreate.end() && (!m_paused && folder.isActive());)
    {
        ...
        if (QDir().mkdir(folderPath) || fileInfo.exists())
        {
            ...
            folderIt = folder.foldersToCreate.erase(static_cast<QHash<hash64_t, QPair<QByteArray, qint32>>::const_iterator>(folderIt));
